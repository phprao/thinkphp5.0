...
class AgentsStatisticsDayModel extends Model
{
    ...
    public function getSubPlayerIn($condition = null,$agent_id){

        $this->where('statistics_player_id',$agent_id);
        $this->where('statistics_time','>=',$condition['start']);
        $this->where('statistics_time','<=',$condition['end']);
        $this->field([
            'SUM(statistics_income) as total ,statistics_player_id',
        ]);
